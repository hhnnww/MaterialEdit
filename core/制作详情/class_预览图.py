...
from core.setting import MATERIAL_FILE_SUFFIX
from core.素材文件夹功能.fun_指定遍历 import fun_指定遍历


class XQPreviewPic:
    def __init__(self, img_list: List[Path], material_path: Path):
    ...
    def fun_制作信息(stem: str, size_info: str):
        stem_pil = PICEdit.fun_单行文字(text=stem.upper(), font_weight='r', font_size=22, text_color=(120, 120, 120),
                                        bg_color=(255, 255, 255)).main()
        size_info = PICEdit.fun_单行文字(text=size_info.upper(), font_weight='r', font_size=18,
                                         text_color=(120, 120, 120),
                                         bg_color=(255, 255, 255)).main()
        bg_width = max([stem_pil.width, size_info.width])
        bg_height = sum([stem_pil.height, size_info.height]) + 120
    ...
    def main(self):
        ...
        for img in self.img_list:
            ...
            if ma_file is not None:
                info_pil = self.fun_制作信息(ma_file, f'{small_im.ori_width}*{small_im.ori_height}(PX)')
